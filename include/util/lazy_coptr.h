 ...
 */
template <typename T> class lazy_coptr {
 public:
  lazy_coptr() {}

  explicit lazy_coptr(std::function<T*()> Constructor) { Init(Constructor); }

  lazy_coptr(lazy_coptr&& rhs) {
    obj = std::move(rhs.obj);
    func = std::move(rhs.func);
  }

  lazy_coptr& operator=(lazy_coptr&& rhs) {
    obj = std::move(rhs.obj);
    func = std::move(rhs.func);
  }

  lazy_coptr(lazy_coptr&) = delete;
  lazy_coptr& operator=(lazy_coptr&) = delete;
  ...
  const std::unique_ptr<T>& operator->() const {
    make(true);
    assert(obj != nullptr && "Null dereference through lazy_coptr.");
    return obj;
  }
