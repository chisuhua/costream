...
#include "co/sync/channel_impl.h"
...
public:
    ...
    {
        DebugPrint1(dbg_mask_ + "dbg_channel", "[id=%ld] Channel init. capacity=%lu", this->getId(), capacity);
    }
    
    // write
    bool Push(T t, bool bWait, FastSteadyClock::time_point deadline = FastSteadyClock::time_point{})
    {
        DebugPrint1(dbg_channel, "[id=%ld] Push ->", this->getId());

        if (closed_) {
            DebugPrint1(dbg_channel, "[id=%ld] Push by closed", this->getId());
            return false;
        }
            ...
            if (wait & readMask) {
                            ...
                            }))
                {
                    if (++spin >= kSpinCount) {
                        spin = 0;
//                            printf("spin by push\n");
                        Processor::StaticCoYield();
                    }

                    if (capacity_ == 0) {
                        if (closed_) {
                            DebugPrint1(dbg_channel, "[id=%ld] Push failed by closed.", this->getId());
                            return false;
                        } else if (!bWait) {
                            DebugPrint1(dbg_channel, "[id=%ld] TryPush failed.", this->getId());
                            return false;
                        }
                    }
                    continue;
                }
                DebugPrint1(dbg_channel, "[id=%ld] Push Notify.", this->getId());
        ...
        auto cond = [&](size_t size) -> typename cond_t::CondRet {
            ...
            if (size < capacity_) {
                ret.needWait = false;
                DebugPrint1(dbg_channel, "[id=%ld] Push no wait.", this->getId());
                return ret;
            }
            ...
            DebugPrint1(dbg_channel, "[id=%ld] Push wait.", this->getId());
            return ret;
        };
        ...
        switch ((int)cv_status) {
            case (int)cond_t::cv_status::no_timeout:
                ...
                DebugPrint1(dbg_channel, "[id=%ld] Push complete.", this->getId());
                return true;

            case (int)cond_t::cv_status::timeout:
                DebugPrint1(dbg_channel, "[id=%ld] Push timeout.", this->getId());
                wait_ -= write1;
                return false;
            ...
            case (int)cond_t::cv_status::no_queued:
                if (closed_)
                    ...
                    DebugPrint1(dbg_channel, "[id=%ld] Push failed.", this->getId());
                wait_ -= write1;
                return false;
    ...
    {
        DebugPrint1(dbg_channel, "[id=%ld] Pop ->", this->getId());
        ...
        for (;;) {
            ...
            if (wait & writeMask) {
                            ...
                            }))
                {
                    ...
                    if (closed_) {
                        DebugPrint1(dbg_channel, "[id=%ld] Pop failed by closed.", this->getId());
                        return false;
                    } else if (!bWait) {
                        DebugPrint1(dbg_channel, "[id=%ld] TryPop failed.", this->getId());
                        return false;
                    }
                    continue;
                }

                DebugPrint1(dbg_channel, "[id=%ld] Pop Notify.", this->getId());
        ...
        auto cond = [&](size_t size) -> typename cond_t::CondRet {
            ...
            DebugPrint1(dbg_channel, "[id=%ld] Pop wait.", this->getId());
            return ret;
        };
        typename cond_t::cv_status cv_status;
        if (deadline == time_point_t())
        ...
        switch ((int)cv_status) {
            case (int)cond_t::cv_status::no_timeout:
                ...
                DebugPrint1(dbg_channel, "[id=%ld] Pop complete.", this->getId());
                return true;

            case (int)cond_t::cv_status::timeout:
                DebugPrint1(dbg_channel, "[id=%ld] Pop timeout.", this->getId());
                wait_ -= read1;
                return false;
            ...
            case (int)cond_t::cv_status::no_queued:
                if (closed_)
                    ...
                    DebugPrint1(dbg_channel, "[id=%ld] Pop failed.", this->getId());
                wait_ -= read1;
                return false;
    ...
    ~CASChannelImpl() {
        DebugPrint1(dbg_mask_ + "dbg_channel", "[id=%ld] Channel destory.", this->getId());
    }

    void SetDbgMask(std::string mask) {
        dbg_mask_ = mask;
    }
    ...
    void Close()
    {
        ...
        DebugPrint1(dbg_mask_ + "dbg_channel", "[id=%ld] Channel Closed. size=%d", this->getId(), (int)Size());
