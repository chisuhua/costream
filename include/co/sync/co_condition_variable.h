...
private:
    ...
    struct Entry : public WaitQueueHook, public RefObject
    {
        ...
        Processor::SuspendEntry coroEntry;
        ...
        bool notify(Functor const func) {
            ...
            if (!nativeThreadEntry) {
                ...
                if (Processor::Wakeup(coroEntry, [&]{ if (func) func(value); })) {
...
private:
    ...
    inline void coroSuspend(Processor::SuspendEntry & coroEntry, TimeType * time)
    {
        if (time)
            coroEntry = Processor::Suspend(*time);
        else
            coroEntry = Processor::Suspend();
    }
