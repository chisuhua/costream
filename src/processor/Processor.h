...
private:
    ...
    // queue_t
    core::IQueue* up_queue_;          // receive from scheduler
    core::IQueue* down_queue_;        // send to scheduler
    size_t up_queue_size_ {4096};
    size_t down_queue_size_ {4096};
...
public:
    ...
    struct SuspendEntry {
        ...
        bool IsExpire() const {
            return Processor::IsExpire(*this);
        }
    };
...
private:
    explicit Processor(Scheduler * scheduler, int id,  core::IQueue* up_queue, core::IQueue* down_queue);
...
ALWAYS_INLINE void Processor::StaticCoYield()
{
    auto proc = GetCurrentProcessor();
    if (proc) proc->CoYield();
}

ALWAYS_INLINE void Processor::CoYield()
{
